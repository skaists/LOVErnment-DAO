//! D-008r2 — Atomic pipeline suite (durable idempotence rework)
//!
//! VOICE-1 §5.5: no utterance without its entry. The pipeline is
//! atomic: create pending → submit post → finalize.
//! Durable idempotence: the audit trail is the lock — the PDS is
//! queried before the in-process mark.
...
/// The store maps derivation_input → (PendingEntry, Option<post_uri>, Option<post_cid>).
...
impl DurableMockPds {
    ...
    fn from_store(store: HashMap<String, (PendingEntry, Option<String>, Option<String>)>) -> Self {
        Self {
...
impl PdsClient for DurableMockPds {
    ...
    fn create_pending_entry(&mut self, key: &str, entry: &PendingEntry) -> Result<(), String> {
        self.store.borrow_mut().insert(key.to_string(), (entry.clone(), None, None));
        Ok(())
    }
...
fn happy_path_writes_both_entry_finalized() {
    ...
    let result = pipeline.run(
        ...
        HeartbeatState::Alive, &FixedClock, &Fnv1aHasher,
    );
    ...
    assert!(pds.finalize_result.as_ref().map_or(false, |r| r.is_ok()),
        "finalize must have succeeded");
    // Verify the store holds a finalized entry.
    let store = pds.store.borrow();
    let entry = store.get("skaists/LOVErnment-DAO@884b2bce");
    assert!(entry.is_some(), "store must contain the entry");
    let (_, uri, cid) = entry.unwrap();
    assert!(uri.is_some(), "entry must be finalized with post_uri");
    assert!(cid.is_some(), "entry must be finalized with post_cid");
}
...
fn wrapper_refused_writes_neither() {
    ...
    assert!(pds.store.borrow().is_empty(), "no entry persisted on refusal");
}
...
fn post_failure_removes_pending_entry() {
    ...
    assert!(pds.removed.borrow().is_some(),
        "pending entry must be removed on post failure");
    assert!(pds.store.borrow().is_empty(),
        "store must be empty after pending removal");
}
...
fn finalize_failure_leaves_pending_entry_alive() {
    ...
    // The pending entry MUST survive in the store — detectable honesty.
    let store = pds.store.borrow();
    let entry = store.get("skaists/LOVErnment-DAO@884b2bce");
    assert!(entry.is_some(), "pending entry must survive finalize failure");
    let (_, uri, _) = entry.unwrap();
    assert!(uri.is_none(), "entry must be unfinalized (pending)");
}
...
fn rerun_after_finalize_failed_refuses() {
    ...
    assert!(matches!(result1, PipelineResult::FinalizeFailed { .. }));

    let mut pds_ok = DurableMockPds::from_store(pds_fail.store.into_inner());
    ...
    assert_eq!(
        ...
        "rerun after FinalizeFailed must refuse — the pending entry in \
...
fn rerun_after_post_failed_refuses() {
    ...
    assert_eq!(
        ...
        "rerun after PostFailed must refuse — idempotence on attempt \
         (in-process mark set before submit)"
    );
}
...
#[test]
fn r1_restart_simulator_fresh_pipeline_same_store_refuses() {
    ...
    let result_b = pipeline_b.run(
        &clean_facts(), &mut pds_b, &mut counter_b,
        HeartbeatState::Alive, &FixedClock, &Fnv1aHasher,
    );
    assert_eq!(
        result_b, PipelineResult::Duplicate,
        "fresh pipeline over the same store must refuse — the audit \
         trail is the durable lock, not the in-process mark"
    );
    assert!(pds_b.submitted.borrow().is_none(), "no post attempt on restart");
}
